...
export default class GetOpportunitiesScoreCommand extends IncentivesCommandBase {
  ...
  async run(): Promise<void> {
    ...
    if (targets.length !== applicableGroups.length) {
      this.error(
        `The amount of groups and targets should be equal: Targets: ${targets.length} Got: ${applicableGroups.length}!`,
        {
...
with:`
    ...
    for (let workingGroup of allWorkingGroups) {
      ...
      if (indexOfGroup != -1) {
        ...
        this.log(
          `The WORKER_OPPORTUNITIES_SCORE for ${workingGroup.toString()}, given a target of ${
            100 - target
